 ...
 */
import { runSqrl, buildServices, buildTestTrace } from "./helpers/sqrlTest";
...
LET UniquesByIp := countUnique(${countStatement} GROUP BY Ip LAST WEEK);
    ...
    await rv.lastState.fetchByName("SqrlExecutionComplete");
    return {
      state: rv.lastState,
      manipulator: rv.lastManipulator
    };
  };
  const ctx = buildTestTrace();

  ({ state, manipulator } = await run("SessionActor"));
  expect(await state.fetchByName("UniquesByIp")).toEqual(1);
  await manipulator.mutate(ctx);
  ({ state, manipulator } = await run("SessionActor", {
    SessionActor: "Actor2"
  }));
  expect(await state.fetchByName("UniquesByIp")).toEqual(2);
  await manipulator.mutate(ctx);
  ...
  expect(await state.fetchByName("UniquesByIp")).toEqual(3);
  await manipulator.mutate(ctx);
